...
\title{Dams Package Demo}
\author{Gopi Goteti}
\maketitle

This document provides examples on how to obtain and extract data using the dams package and how to create summary graphics of the extracted data.

\section{Data Attributes}

If you have not already done so, load the package along with RCurl (for data extraction from bitbucket.org), ggplot and maps (for graphics).
...
Read sample data and get the columns names.

<<>>=
dams_sample <- nid_cleaned
...
\section{Enitre Dataset}

Get the entire dataset. This might take a few moments.
...
\begin{figure}
\begin{center}
<<label = fig1, width=8, height=6, fig = TRUE, echo = FALSE>>=
<<fig1plot>>
@
\end{center}
\caption{Number of Dams by Year of Completion}
\label{fig:one}
\end{figure}
...
\begin{figure}
\begin{center}
<<label = fig2, width=8, height=6, fig = TRUE, echo = FALSE>>=
<<fig2plot>>
@
\end{center}
\caption{Number of Dams by State}
\label{fig:two}
\end{figure}

\section{Other Analyses: Flood Control Dams}
